...
export function GallerySliderSection() {
  ...
  const { data: photoGalleries, isLoading: loadingPhotos } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .in("type", ["photo", "photogallery", "photoreport"])
  ...
  const hasPhotos = photoGalleries && photoGalleries.length > 0;
  const hasVideos = videoGalleries && videoGalleries.length > 0;
  ...
  return (
    <section className="py-6">
      <div className={`grid grid-cols-1 ${hasPhotos && hasVideos ? 'md:grid-cols-2' : ''} gap-6`}>
          ...
          <div className="group relative">
            <Link to={currentPhoto ? `/galleries/${currentPhoto.slug}` : "/galleries"} className="block">
              <div className="aspect-[16/10] overflow-hidden bg-muted relative">
                ...
                {/* Navigation Arrows */}
                {photoGalleries && photoGalleries.length > 1 && (
                  <>
                    <Button
                      variant="ghost"
                      size="icon"
        ...
        {/* Video Galleries Slider - only show if videos exist */}
        {hasVideos && (
          <div className="group relative">
            <Link to={currentVideo ? `/galleries/${currentVideo.slug}` : "/galleries"} className="block">
              <div className="aspect-[16/10] overflow-hidden bg-muted relative">
                ...
                {/* Navigation Arrows */}
                {videoGalleries && videoGalleries.length > 1 && (
                  <>
                    <Button
                      variant="ghost"
                      size="icon"
