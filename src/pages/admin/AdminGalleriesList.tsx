...
export default function AdminGalleriesList() {
  ...
  const { data: galleries, isLoading } = useQuery({
    queryKey: ["admin-galleries", search],
    queryFn: async () => {
      let query = supabase
        .from("galleries")
        .select("*")
  ...
  const getImagesCount = (images: unknown) => {
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-condensed font-bold">Галереи</h1>
          <p className="text-muted-foreground">Управление фотогалереями</p>
        </div>
        <Button asChild>
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">
                      {item.type === "gallery" ? "Галерея" : "Репортаж"}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <span className="flex items-center gap-1">
                      <Image className="h-3 w-3" />
                      {getImagesCount(item.images)}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span className="flex items-center gap-1">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button asChild variant="ghost" size="icon">
                        <Link to={`/admin/galleries/${item.id}`}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Удалить галерею?</AlertDialogTitle>
            <AlertDialogDescription>
              Это действие нельзя отменить. Галерея будет удалена навсегда.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
