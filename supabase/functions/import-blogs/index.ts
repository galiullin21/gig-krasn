...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { blogs } = await req.json() as { blogs: BlogEntry[] };

    if (!blogs || !Array.isArray(blogs)) {
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid blogs data' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Starting import of ${blogs.length} blog items...`);
